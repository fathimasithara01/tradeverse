	...
	// RejectWithdrawalRequest rejects a customer's withdrawal request and returns funds to their wallet.
	func (s *AdminWalletService) RejectWithdrawalRequest(withdrawalID uint) error {
		return s.DB.Transaction(func(tx *gorm.DB) error {
			...
			// Get the customer's wallet to return the funds
			customerWallet, err := s.Repo.GetCustomerWallet(withdrawal.UserID) // This needs to be done within the transaction scope or GetCustomerWallet should accept `tx`
			if err != nil {
				// If Repo.GetCustomerWallet does not take `tx`, it will get a non-transactional wallet.
				// It's better if the repository methods use the passed `tx` or the service ensures atomicity.
				// For this specific case, `s.Repo.GetCustomerWallet` is outside the `tx` closure, which is okay for reading.
				// However, `UpdateCustomerWalletBalance` *does* take `tx`, which is correct.
			...
			withdrawal.Status = models.TxStatusRejected                      // Corrected: Using the newly defined constant
			if err := s.Repo.UpdateWithdrawRequest(withdrawal); err != nil { // Corrected: Removed `tx`
				return fmt.Errorf("failed to update withdrawal request status to rejected: %w", err)
			}

			// Create a reversal transaction for the customer
			reversalTx := models.WalletTransaction{
				...
				Status:          models.TxStatusSuccess, // The reversal itself is successful
			...
			// Update the original pending withdrawal transaction to reflect rejection/reversal
			if withdrawal.WalletTransactionID != nil {
				var originalWalletTx models.WalletTransaction
				// Ensure originalWalletTx is fetched within the current transaction `tx`
				if err := tx.First(&originalWalletTx, *withdrawal.WalletTransactionID).Error; err != nil {
					...
					originalWalletTx.Status = models.TxStatusReversed // Mark original as reversed
